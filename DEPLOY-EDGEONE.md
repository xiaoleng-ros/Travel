...
> **密码的存储与传输（2026-09-23 起）**：无论初始创建还是后台改密码，库里存的一律是
> `bcrypt(sha256(盐 + 明文))`；前端在提交前先做客户端哈希，所以 F12 → Network 面板里
> 看不到明文。密码规则为 **6-20 位，大写/小写/数字/特殊符号任选其二**。
> ⚠️ **从旧版本升级上来的库必须跑一次 `scripts/migrate-password-hash.mjs`**
> （先预演，确认后加 `--apply`），否则旧格式记录无法被新前端登录。详见 IMPROVEMENTS.md 第十六节。
...
### 顺带发现：未注册路径的走向取决于 HTTP 方法（文档未提及）

修复后逐路径探测，发现一个官方文档没写的行为。
**必须区分 GET 与 POST**，否则会得出完全相反的结论 —— 我第一轮就搞错了，第二轮才修正：

| 请求 | 结果 |
|------|------|
| `GET /api/health`（已注册） | **200 JSON** ✓ |
| `GET /api/album/public/list`（已注册） | **200 JSON** ✓ |
| `GET /api/admin/me`（已注册，未带 cookie） | **401 JSON** ✓ |
| `GET /api/zzz` / `GET /api/albums` / `GET /api/album`（未注册） | **200 HTML** — 被 SPA 回退接走 |
| `POST /api/admin/nope`（未注册） | **404 JSON** — 进了函数，由 Express 兜底 |
| `POST /api/admin/change-username`（该次尚未部署） | **404 JSON** |

即：

- **未注册的 GET**：在平台层就被「静态资源 + SPA 回退」接走，返回 `index.html`，**到不了 Express**
- **未注册的 POST 及其他非 GET 方法**：没法回退到静态资源，会正常转发进函数，
  由函数末尾的兜底返回 `404 {"code":404,"message":"接口不存在"}`

> ✅ **这条差异很有用**：想判断一个**新增的 POST 接口**是否已经部署上线，
> 直接**未登录**调用它就行 ——
> 返回 `401`（说明进了 `wrapAuth`）= **已上线**；返回 `404 接口不存在` = **函数里还没有这个路由**。
> 全程无损、不需要登录、不消耗登录限流。

**实际影响**：前端若用 **GET** 请求了错误的接口路径，axios 拿到的是 HTML，
会报「网络错误」而不是「接口不存在」；POST 则能拿到明确的 404。排查时别被绕进去。
